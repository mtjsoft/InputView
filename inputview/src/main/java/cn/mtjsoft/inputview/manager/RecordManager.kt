...
class RecordManager {
    ...
    private val DEFAULT_EXECUTOR = ThreadPoolExecutor(
        4, 8, 30, TimeUnit.SECONDS, ArrayBlockingQueue(1024, true)
    )
     ...
     * 开始录制视频
     */
    fun startRecord(savePcmPath: String, saveWavPath: String, maxTime: Long, l: OnRecordListener) {
        RECORD_TIME_MAX = if (maxTime >= 1) maxTime else 60000
        ...
        recordListener = l
        allDuration = 0
        DEFAULT_EXECUTOR.execute(RecordRunnable())
    }
        ...
        override fun run() {
            ...
            try {
                ...
                mAudioRecord?.let {
                    ...
                    outputStream.close()
                    // PCM转WAV
                    pcmToWav(savePcmPath, saveWavPath)
                    if (allDuration > 0 && recordListener != null) {
                        recordListener!!.onRecordOver(allDuration, "", saveWavPath)
                    }
                }
                //录制结束
                stopRecord()
    ...
    private fun pcmToWav(inFilename: String, outFilename: String) {
